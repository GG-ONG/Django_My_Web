...
class TestModel(TestCase):
    def setUp(self):
        self.client=Client()
        self.author_000 = User.objects.create(username='test', password='testpassword')
...
class TestView(TestCase):
    def setUp(self):
        self.client = Client()
        self.author_000 = User.objects.create_user(username='Test', password='testpassword')
        self.user_Test2 = User.objects.create_user(username='Test2', password='testpassword')
    ...
    def test_post_detail(self):
        ...
        comment_000 = create_comment(post_000, text='a test comment', author=self.user_Test2)
        ...
        login_success = self.client.login(username='Test2', password='testpassword')  # login을 한 경우
        self.assertTrue(login_success)
        response = self.client.get(post_000_url)
        self.assertEqual(response.status_code, 200)
        ...
        main_div = soup.find('div', id='main-div')
        self.assertEqual(post_000.author, self.author_000)  # post.author와 login 한 사용자가 동일하면
        self.assertNotIn('EDIT', main_div.text)  # EDIT 버튼이 없다.
